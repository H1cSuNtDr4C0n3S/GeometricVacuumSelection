...
nb = Notebook[{
  Cell["01 - Radiation Era Finite-Domain IR Scaling Test", "Title"],
  Cell["Objective: in radiation era, verify that a finite causal-domain proxy keeps the non-local channel IR-suppressed and with expected H^3 scaling.", "Text"],
...
checkD = Abs[hNormHalf - 1/2] < 5*10^-4;
checkE = Abs[wRef - 1/3] < 0.01 && Abs[wHalf - 1/3] < 0.01;
checkF = zHalf > 0 && zHalf < zRef;
checkG = deltaQRef > 0 && deltaQHalf > 0;
checkH = Abs[ratioDelta/(hNormHalf^3) - 1] < 0.05;
checkI = Abs[ratioI0 hNormHalf^3 - 1] < 0.05;
...
check = TrueQ[classification == "RADIATION_ERA_IR_SUPPRESSED"];

nbName = "01_Radiation_Era_Finite_Domain_IR_Scaling_Test.nb";
logName = "01_radiation_era_finite_domain_ir_scaling_test.log";
logLines = {
  "Notebook: " <> nbName,
  ...
  "checkE(radiation w_eff probes) = " <> ToString[checkE, InputForm],
...
result = <|
  "script" -> "01_radiation_era_finite_domain_ir_scaling_test",
  "nbName" -> nbName,
  "logName" -> logName,
